...
def dmd(X, Y, eps=1e-5):
    ...
    ||TX - Y|| --> min with respect to transition matrix T.
    ...
    X_resh = tf.reshape(X, (-1, X.shape[-1]))
    Y_resh = tf.reshape(Y, (-1, Y.shape[-1]))
    # SVD of X_resh matrix
    lmbd, u, v = tf.linalg.svd(X_resh)
    # number of eigvals > than eps
    ind = tf.reduce_sum(tf.cast(lmbd > eps, dtype=tf.int32))
    # truncation of all elements of the eigendecomposition
    lmbd = lmbd[:ind]
    lmbd_inv = 1 / lmbd
